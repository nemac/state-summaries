...
export default function SandboxCustomSizeExport(props) {
  ...
  const sandboxDefaultExportSizes = SandboxDefaultExportSizes();
  const defaultSizes = {
    name: "Default",
  ...
  const getLabels = (name) => {
    ...
    const labels = sandboxDefaultExportSizes.filter(
  ...
  const getDimensions = (name) => {
    ...
    const dimensionFromButton = sandboxDefaultExportSizes.filter(
      (value) => value.name === name,
    );
  ...
  const swithDimensionActive = (name) =>
    whichDimension === name ? "contained" : "outlined";
